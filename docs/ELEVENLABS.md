...
- `game_context`: default `A peaceful morning at home. Visitors are disabled. Miso the cat, Leo the English cream dachshund and Pebble the tortoise live here.` for dashboard tests.
...
Set `ELEVENLABS_API_KEY` separately in Vercel for occasional personalized greetings. It is used only by `api/voice.js`. Optionally select a companion voice with `ELEVENLABS_VOICE_ID`.
...
Verify after deployment: enter a nickname, hear a welcome, then connect Voice and ask about the pets. The agent should answer from current game context rather than inventing a scene.
