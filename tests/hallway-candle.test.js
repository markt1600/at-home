...
test('blowing creates smoke at the extinguished wicks before replacing the cover; repeated cycles and reset work',()=>{
 const rig=model();rig.activate();advance(rig,9);assert.equal(rig.stage,'lit');rig.activate();advance(rig,.4);assert.equal(rig.flames[0].visible,false);assert.equal(rig.flames[4].visible,true);assert.equal(rig.smoke.visible,true);assert.deepEqual(rig.cover.position,rig.park);
 ...
 rig.activate();advance(rig,9);assert.equal(rig.litCount,5);rig.reset();assert.equal(rig.stage,'covered');assert.deepEqual(rig.cover.position.toArray(),[0,0,0]);assert.equal(rig.litCount,0);assert.ok(rig.flames.every(f=>!f.visible));
});
