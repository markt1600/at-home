...
function panelHTML(type){
 ...
 if(type==='memory'){const m=memories.find(m=>m.id===selectedMemory);if(!m)return '';const count=memoryMedia(m).length;return `<div class="dream"><div class="memory-media"></div><div class="dream-caption"><p class="eyebrow">${m.date?escape(formatMemoryDate(m.date)):'A moment to come back to'}</p><h2>${escape(m.title)}</h2><p class="memory-caption">${escape(m.description)}</p>${count===1&&m.soundtrack?'<div class="slideshow-controls"><button data-slide="pause">Pause memory</button></div>':''}${count>1?'<div class="slideshow-controls"><button data-slide="previous" aria-label="Previous photo">←</button><span class="slide-counter" aria-live="off"></span><button data-slide="pause">Pause slideshow</button><button data-slide="next" aria-label="Next photo">→</button></div>':''}<p class="fine" id="memory-error">${count>1?'Photos every 2 seconds · ← → to browse · ':''}${count>1||m.type==='video'||m.soundtrack?'Space to pause · ':''}Esc to return</p><button data-action="resume" class="dream-return">Return to the house ↗</button></div></div>`;}
