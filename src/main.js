...
function updateLookHint(id){const el=$('#look-hint');if(el)el.textContent=id==='turntable'?`E · ${recordPlayer.enabled?'Stop the record':'Play the record'}`:id?`E · ${PETS.find(p=>p.id===id)?.name||NEIGHBORS.find(n=>n.id===id)?.name}`:'';}
...
async function enter(){
 playing=true;world.mode='play';world.paused=false;world.hours=state.hours;world.syncPeople(state);save();renderHUD();world.resumeWandering();
...
function openPanel(type){
 ...
 if(type==='memory'){
  ...
  const m=memories.find(m=>m.id===selectedMemory);if(m)memoryPlayer=mountMemoryPlayer($('#panel-content'),m,{onError:message=>{const el=$('#memory-error');if(el)el.textContent=message;}});
 }
}
...
function closePanel(resume=true){if(!panel)return;const wasMemory=panel==='memory';if(wasMemory)releaseMemory();$('#panel').close();panel=null;world.paused=false;world.previewAnimation=false;speech.stop();save();if(resume||wasMemory)world.resumeWandering();}
...
function panelHTML(type){
 ...
 if(type==='memory'){const m=memories.find(m=>m.id===selectedMemory);if(!m)return '';const count=memoryMedia(m).length;return `<div class="dream"><div class="memory-media"></div><div class="dream-caption"><p class="eyebrow">${m.date?escape(formatMemoryDate(m.date)):'A moment to come back to'}</p><h2>${escape(m.title)}</h2><p class="memory-caption">${escape(m.description)}</p>${count>1?'<div class="slideshow-controls"><button data-slide="previous" aria-label="Previous photo">←</button><span class="slide-counter" aria-live="off"></span><button data-slide="pause">Pause slideshow</button><button data-slide="next" aria-label="Next photo">→</button></div>':''}<p class="fine" id="memory-error">${count>1?'Photos every 2 seconds · ← → to browse · ':''}${count>1||m.type==='video'?'Space to pause · ':''}Esc to return</p><button data-action="resume" class="dream-return">Return to the house ↗</button></div></div>`;}
...
app.addEventListener('click',async e=>{
 ...
 switch(d.action){
  ...
  case 'leave':recordPlayer.stop();save();closePanel(false);await companion.disconnect();speech.stop();sound.setVolume(0);playing=false;world.mode='menu';renderMenu();break;
...
document.addEventListener('keydown',e=>{
 ...
 if(world.lookTarget){const id=world.lookTarget;if(id==='turntable'){toggleRecord();return;}if(PETS.some(p=>p.id===id)){selectedPet=id;world.lookAtPet(id);openPanel('pets');}else{selectedNeighbor=id;openPanel('neighbors');speakNeighbor(id,'greeting');}return;}
...
if(import.meta.env.DEV)window.__atHome={world,recordPlayer,get state(){return state;},setTime(hours){state.hours=hours;world.hours=hours;},visit(id='ken'){state.pending=id;world.syncPeople(state);updateHUD();}};
