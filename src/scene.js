...
export class House{
 ...
 loadArtwork(){const loader=new THREE.TextureLoader(),a=Math.min(8,this.renderer.capabilities.getMaxAnisotropy());loader.load('/art/dining-prints.webp',t=>applyDiningArt(t,this.houseMaterials,a));loader.load('/art/entry-details-atlas.webp',t=>applyHomeDetails(t,this.houseMaterials,a));loader.load('/art/house-surfaces-v2.png',t=>applyHouseTextures(t,this.houseMaterials,a));loader.load('/art/hallway-prints.webp',t=>applyHallwayArt(t,this.houseMaterials,a));loader.load('/art/powder-marble.webp',t=>applyBathroomTextures(t,this.houseMaterials,a));fetch('/art/friends/framing.json').then(r=>{if(!r.ok)throw Error();return r.json();}).then(f=>{this.framing=f;this.syncPeople(this.life);}).catch(()=>this.onAssetError?.());}
