...
export class House{
 constructor(canvas,onLook=()=>{},onTick=()=>{}){
  ...
  this.petRoaming=new PetRoaming(this.colliders);this.balconyLife=new BalconyLife(this);this.loadArtwork();this.focus('living');this.resize();this.clock=new THREE.Clock();
 ...
 animate(){requestAnimationFrame(()=>this.animate());const dt=Math.min(this.clock.getDelta(),.05);this.elapsed+=dt;const active=this.mode==='play'&&!this.paused&&!document.hidden;this.walking=false;
  ...
  this.petRoaming.update(active?dt:0,{enabled:this.motion,player:this.camera.position,hours:this.hours});for(const [id,p] of this.petRoaming.pets){const actor=this.actors.get(id);if(actor){actor.position.set(p.x,p.y,p.z);actor.userData.walking=p.moving;}}
