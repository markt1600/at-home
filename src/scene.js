...
export class House{
 ...
 buildPetCorners(){this.petCorners=new Map();for(const p of PETS){const [x,z]=planPoint(...(p.carePlan||p.plan)),y=floorHeight(x,z),g=new THREE.Group();g.name=p.name+' care corner';g.position.set(x,y+.012,z);this.scene.add(g);const size=p.carePlan?.20:p.id==='sunny'?.48:p.id==='miso'?.32:.26;this.cyl(size,size,.018,0,0,0,this.mat(p.id==='pebble'?0xc1b596:0xb7c19b),g,48);const contents=[];for(const [i,color] of [0x755239,0x8eafb6].entries()){const px=size+.17,pz=(i-.5)*.30;const bowl=this.cyl(.135,.115,.045,px,.022,pz,this.mat(0xabb69e,.35),g,32);const liquid=this.cyl(.113,.113,.006,px,.047,pz,this.mat(color,.2),g,32);contents.push(liquid);if(i===0)for(let j=0;j<9;j++){const a=j*2.4,r=.025+(j%3)*.026;this.sphere(p.id==='pebble'?.027:.012,px+Math.cos(a)*r,.058,pz+Math.sin(a)*r,this.mat(p.id==='pebble'?0x63854a:0x976c40),g,1,p.id==='pebble'?.18:1,1.5);}}
 ...
 syncPets(s){this.life=s;if(!s||!this.framing)return;const specs=PETS.map(p=>({...p,position:planPoint(...p.plan)}));
  ...
  for(const spec of specs){if(!this.framing[spec.id])continue;let g=this.actors.get(spec.id);if(!g){g=createActor(spec.id,spec.height,this.framing[spec.id],this.motionFraming);this.actors.set(spec.id,g);this.scene.add(g);const p=this.petRoaming.register(spec.id,spec.plan);g.position.set(p.x,p.y,p.z);}}
 }
 ...
 animate(){requestAnimationFrame(()=>this.animate());const dt=Math.min(this.clock.getDelta(),.05);this.elapsed+=dt;const active=this.mode==='play'&&!this.paused&&!document.hidden;this.walking=false;
  ...
  let petAim=0;for(const [id,g] of this.actors){const distance=g.position.distanceTo(this.camera.position);g.rotation.y=Math.atan2(this.camera.position.x-g.position.x,this.camera.position.z-g.position.z);g.userData.update(animated?dt:0,this.motion,distance<9);const d=g.position.clone().add(new THREE.Vector3(0,PETS.find(p=>p.id===id)?.height/2||1.3,0)).sub(this.camera.position),aim=forward.dot(d.clone().normalize());if(distance<3.3&&aim>.85&&aim>petAim){const ray=new THREE.Raycaster(this.camera.position,d.clone().normalize(),.05,d.length()-.08);if(!ray.intersectObject(this.houseRoot,true).some(hit=>!hit.object.material.transparent)){nearest=id;petAim=aim;}}}
  ...
  if(this.turntablePosition){const d=this.turntablePosition.clone().sub(this.camera.position);if(d.length()<2.3&&forward.dot(d.clone().normalize())>.72){const ray=new THREE.Raycaster(this.camera.position,d.clone().normalize(),.05,d.length()-.30);if(!ray.intersectObject(this.houseRoot,true).some(hit=>!hit.object.material.transparent))nearest='turntable';}}
  if(this.windowLounge?.telescope){const d=this.windowLounge.telescope.position.clone().add(new THREE.Vector3(0,1.49,.54)).sub(this.camera.position);if(d.length()<2.5&&forward.dot(d.clone().normalize())>.68){const ray=new THREE.Raycaster(this.camera.position,d.clone().normalize(),.05,Math.max(.05,d.length()-.25));if(!ray.intersectObject(this.houseRoot,true).some(hit=>!hit.object.material.transparent))nearest='telescope';}}
