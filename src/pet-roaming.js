...
export class PetRoaming{
 ...
 greet(player,yaw=0){
  for(const [i,id] of ['sunny','miso'].entries()){
   const p=this.pets.get(id);if(!p)continue;const home=this.nodes.get(p.home);Object.assign(p,{x:home.x,y:home.y,z:home.z,path:[],wait:i*.7,blocked:0,moving:false});
 ...
 update(dt,{enabled=true,player=null,hours=12}={}){
  ...
  for(const p of this.pets.values()){
   ...
   if(Math.hypot(next.x-p.x,next.z-p.z)<.002){p.path.shift();if(!p.path.length){const night=hours%24<6||hours%24>21;p.wait=(p.greeting?12:night?14:4)+this.random()*(p.id==='pebble'?14:10);p.greeting=null;}}
  }
 }
