...
async function refresh(){
 ...
 try{session=await api('?action=session');if(session.authenticated)items.push(...await api('?admin=1'));}catch{session={};}
 render();
}
...
function render(){
 ...
 <div class="workspace"><aside><button class="primary" data-action="new">+ Add a memory</button><div class="library">${items.map(i=>`<button data-id="${esc(i.id)}" class="${i.id===selected?'selected':''}"><strong>${esc(i.title)}</strong><small>${i.cloud?(i.published?'Published':'Private draft'):'This device'}${i.date?' · '+esc(formatMemoryDate(i.date)):''}</small></button>`).join('')||'<p>Your first memory starts here.</p>'}</div></aside><section class="editor">${editorContent(m)}</section></div></main>`;
