...
export function buildEntranceIslandDetails(world,console,m){
 ...
 const bronze=world.mat(0x59503a,.36,.66),green=world.mat(0x173c2b,.3,.28),ivory=world.mat(0xf4ead6,.24,.04),wax=world.mat(0x938575,.87);
 const mesh=(parent,geo,mat,x=0,y=0,z=0)=>{const o=new THREE.Mesh(geo,mat);o.position.set(x,y,z);o.castShadow=o.receiveShadow=true;parent.add(o);return o;};
 ...
 world.cyl(.034,.042,.050,0,.35,0,bronze,lamp,24);
 for(let i=0;i<8;i++){
 ...
 world.cyl(.055,.065,.012,0,.629,0,bronze,lamp,32);
 world.cyl(.009,.017,.022,0,.647,0,bronze,lamp,16);world.sphere(.012,0,.661,0,bronze,lamp,1,.4,1);
 ...
 world.cyl(.126,.126,.012,0,.339,0,wax,candle,48);
 for(const [x,z] of [[-.047,-.029],[.048,-.025],[0,.052]])world.cyl(.0018,.002,.013,x,.35,z,m.black,candle,6);
 ...
 const cloche=lathe(candle,clochePoints,glass);cloche.name='Transparent rounded candle dome';cloche.castShadow=false;ring(candle,.172,.003,.007,glass).castShadow=false;
 ...
 for(const side of [-1,1])world.cyl(.020,.020,.066,side*.112,.211,0,ivory,vase,20).rotation.z=Math.PI/2;
 tube(vase,[[-.113,.231,.008],[-.116,.192,.024],[-.117,.136,.03],[-.096,.114,.028]],.002,world.mat(0x842d31,.78));
