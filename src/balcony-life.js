...
export class BalconyLife{
 constructor(world){
  ...
  const [x,z]=planPoint(...FEEDER_PLAN),[tx,tz]=planPoint(260,610);this.perch=new THREE.Vector3(x-.13,2.405,z);this.tree=new THREE.Vector3(tx,1.9,tz);
 ...
 update(dt,hours,motion=true){
  this.time+=dt;const h=((hours%24)+24)%24;this.root.visible=h>6&&h<19.1;if(!this.root.visible)return;
