...
export function installPetBone(world){
 ...
 world.houseInteractions.add({id:'leo-bone',pos:position,range:2.65,touchRadius:.20,label:()=>"Throw Leo’s bone",available:()=>bone.canThrow(world.camera.position),activate:()=>bone.throw(world.camera.position,world.camera.getWorldDirection(new THREE.Vector3()))});
 bone.mesh=group;
 bone.updateModel=()=>{
