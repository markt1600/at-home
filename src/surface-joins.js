...
function split(poly,a,b){
 ...
 for(let i=0;i<poly.length;i++){
  ...
  if((ds>=0)!==(de>=0)){
   const t=ds/(ds-de),v={};for(const key of ['p','n','uv','q'])v[key]=s[key].map((x,j)=>x+(e[key][j]-x)*t);
...
export function resolveSurfaceJoins(records){
 ...
 for(const record of records){
  ...
  const g=record.geometry,p=g.attributes.position,n=g.attributes.normal,uv=g.attributes.uv,output=[];let changed=false;
  for(let face=0;face<6;face++){
   ...
   const polygon=[0,1,4,2].map(j=>{const i=face*6+j,pos=new THREE.Vector3().fromBufferAttribute(p,i);return{p:pos.toArray(),n:[n.getX(i),n.getY(i),n.getZ(i)],uv:[uv.getX(i),uv.getY(i)],q:[pos.dot(u),pos.dot(v)]};});
